...
def my_log_likehood(posterior_samples,X,Y):
    ...
    p_true=np.minimum(np.maximum(jnp.exp(jnp.sum(dist.Normal(mu1,sigma1).log_prob(X),axis=-1))/(0.5*jnp.exp(jnp.sum(dist.Normal(mu0,sigma0).log_prob(X),axis=-1))+0.5*jnp.exp(jnp.sum(dist.Normal(mu1,sigma1).log_prob(X),axis=-1))),0.0001),0.9999)
...
# create artificial regression dataset
    ...
    np.save("./npy_files/bayes_nb_zero.npy",{"posterior_samples":posterior_samples,"y_fitted_dist":y_fitted_dist,"y_predictive_dist":y_predictive_dist,"target_train":target_train,"target_test":target_test,"num_chains":NUM_CHAINS,"num_bayes_samples":NUM_BAYES_SAMPLES,"divergences":divergences})
