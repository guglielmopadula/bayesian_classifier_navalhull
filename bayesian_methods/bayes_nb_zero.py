...
if __name__ == "__main__":
  ...
  for (n in 1:N)
    x[n] ~ normal(mu[y[n]+1],sigma[y[n]+1]);
  }
  ...
  for (n in 1:N){
  p_new[n]=exp(normal_lpdf(x_new[n]|to_vector(mu[2]),sigma[2]))/(exp(normal_lpdf(x_new[n]|mu[1],sigma[1]))+exp(normal_lpdf(x_new[n]|mu[2],sigma[2])));
  y_new[n] = bernoulli_rng(p_new[n]);}
  for (n in 1:N){
  p_pred[n]=exp(normal_lpdf(x[n]|to_vector(mu[2]),sigma[2]))/(exp(normal_lpdf(x[n]|mu[1],sigma[1]))+exp(normal_lpdf(x[n]|mu[2],sigma[2])));
  ...
  np.save("./npy_files/bayes_nb_zero.npy",{"fit":fit,"y_fitted_dist":y_fitted_dist,"y_predictive_dist":y_predictive_dist,"target_train":target_train,"target_test":target_test,"num_chains":4,"num_bayes_samples":1000,"num_warmup":1000,"divergences":divergences})
