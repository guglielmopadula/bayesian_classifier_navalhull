...
def predict(mu,sigma,X):
    ...
    sigma1=sigma1.reshape(NUM_CHAINS*NUM_BAYES_SAMPLES,1,D_X,D_X)
    sigma0=sigma0.reshape(NUM_CHAINS*NUM_BAYES_SAMPLES,1,D_X,D_X)
    p_true=jnp.exp(dist.MultivariateNormal(mu1,sigma1).log_prob(X))/(0.5*jnp.exp(dist.MultivariateNormal(mu0,sigma0).log_prob(X))+0.5*jnp.exp(dist.MultivariateNormal(mu1,sigma1).log_prob(X)))
...
def my_log_likehood(posterior_samples,X,Y):
    ...
    sigma=posterior_samples["sigma"].reshape(-1,2,D_X,D_X)
    mu0=mu[:,0,:]
    mu1=mu[:,1,:]
    sigma0=sigma[:,0,:,:]
    sigma1=sigma[:,1,:,:]
    ...
    sigma1=sigma1.reshape(-1,1,D_X,D_X)
    sigma0=sigma0.reshape(-1,1,D_X,D_X)
    p_true=np.minimum(np.maximum(jnp.exp(dist.MultivariateNormal(mu1,sigma1).log_prob(X))/(0.5*jnp.exp(dist.MultivariateNormal(mu0,sigma0).log_prob(X))+0.5*jnp.exp(dist.MultivariateNormal(mu1,sigma1).log_prob(X))),0.000001),0.999999)
...
# create artificial regression dataset
    ...
    sigma=posterior_samples["sigma"].reshape(NUM_CHAINS*NUM_BAYES_SAMPLES,2,D_X,D_X)
    ...
    np.save("./npy_files/bayes_qda_zero.npy",{"posterior_samples":posterior_samples,"y_fitted_dist":y_fitted_dist,"y_predictive_dist":y_predictive_dist,"target_train":target_train,"target_test":target_test,"num_chains":NUM_CHAINS,"num_bayes_samples":NUM_BAYES_SAMPLES,"divergences":divergences})
