...
def model(X,Y):
    N,D_X=X.shape
    bias0 = numpyro.sample("bias0", dist.Normal(np.zeros((1, bias0_prior.shape[0])), 0.5*np.ones((1, bias0_prior.shape[0]))))#0.5
    bias1 = numpyro.sample("bias1", dist.Normal(np.zeros((1, bias1_prior.shape[0])), 0.5*np.ones((1, bias1_prior.shape[0]))))#0.5
    bias2 = numpyro.sample("bias2", dist.Normal(np.zeros((1, bias2_prior.shape[0])), 0.5*np.ones((1, bias2_prior.shape[0]))))#0.5
    matrix0 = numpyro.sample("matrix0", dist.Normal(jnp.zeros((matrix0_prior.shape[0], matrix0_prior.shape[1])),0.3*jnp.ones((matrix0_prior.shape[0], matrix0_prior.shape[1]))))
    matrix1 = numpyro.sample("matrix1", dist.Normal(jnp.zeros((matrix1_prior.shape[0], matrix1_prior.shape[1])),0.3*jnp.ones((matrix1_prior.shape[0], matrix1_prior.shape[1]))))
    matrix2 = numpyro.sample("matrix2", dist.Normal(jnp.zeros((matrix2_prior.shape[0], matrix2_prior.shape[1])),0.3*jnp.ones((matrix2_prior.shape[0], matrix2_prior.shape[1]))))
...
if __name__ == "__main__":
    ...
    np.save("./npy_files/bayesian_nn_zero.npy",{"posterior_samples":posterior_samples,"y_fitted_dist":y_fitted_dist,"y_predictive_dist":y_predictive_dist,"target_train":target_train,"target_test":target_test,"num_chains":NUM_CHAINS,"num_bayes_samples":NUM_BAYES_SAMPLES,"num_warmup":NUM_WARMUP,"divergences":divergences})
