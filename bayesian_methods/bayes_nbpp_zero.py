...
def model(X,Y):
    ...
    sigma=numpyro.sample("sigma",dist.HalfNormal(jnp.ones((D_X))))
    x_hat=numpyro.sample("x_hat",dist.Normal(mu[Y],sigma).to_event(1),obs=X) 
    return x_hat   
...
def predict(mu,sigma,X):
    ...
    p_true=jnp.exp(jnp.sum(dist.Normal(mu1,sigma).log_prob(X),axis=-1))/(0.5*jnp.exp(jnp.sum(dist.Normal(mu0,sigma).log_prob(X),axis=-1))+0.5*jnp.exp(jnp.sum(dist.Normal(mu1,sigma).log_prob(X),axis=-1)))
...
def my_log_likehood(posterior_samples,X,Y):
    ...
    sigma=posterior_samples["sigma"].reshape(-1,1,D_X)
    mu0=mu[:,0,:]
    mu1=mu[:,1,:]
    mu0=mu0.reshape(-1,1,D_X)
    mu1=mu1.reshape(-1,1,D_X)
    p_true=np.minimum(np.maximum(jnp.exp(jnp.sum(dist.Normal(mu1,sigma).log_prob(X),axis=-1))/(0.5*jnp.exp(jnp.sum(dist.Normal(mu0,sigma).log_prob(X),axis=-1))+0.5*jnp.exp(jnp.sum(dist.Normal(mu1,sigma).log_prob(X),axis=-1))),0.0001),0.9999)
...
# create artificial regression dataset
    ...
    np.save("./npy_files/bayes_nbpp_zero.npy",{"posterior_samples":posterior_samples,"y_fitted_dist":y_fitted_dist,"y_predictive_dist":y_predictive_dist,"target_train":target_train,"target_test":target_test,"num_chains":NUM_CHAINS,"num_bayes_samples":NUM_BAYES_SAMPLES,"divergences":divergences})
