...
def model(X,Y):
    N,D_X=X.shape
    alpha = numpyro.sample("alpha", dist.Normal(0, 5*np.ones((1, 1))))#0.001
    beta = numpyro.sample("beta", dist.Normal(0,5*jnp.ones((D_X, 1))))#0.001
...
np.save("bayes_logreg_zero.npy",{"posterior_samples":posterior_samples,"y_fitted_dist":y_fitted_dist,"y_predictive_dist":y_predictive_dist,"target_train":target_train,"target_test":target_test,"num_chains":NUM_CHAINS,"num_bayes_samples":NUM_BAYES_SAMPLES,"num_warmup":NUM_WARMUP})
