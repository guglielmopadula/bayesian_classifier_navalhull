...
model {
  ...
  for (n in 1:N)
      x[n] ~ multi_normal(to_vector(mu[y[n]+1]),quad_form_diag(L*L', to_vector(sigma)));
}
...
for (n in 1:N){
  p_new[n]=exp(multi_normal_lpdf(x_new[n]|to_vector(mu[2]),quad_form_diag(L*L', to_vector(sigma))))/(exp(multi_normal_lpdf(x_new[n]|mu[1],quad_form_diag(L*L', to_vector(sigma))))+exp(multi_normal_lpdf(x_new[n]|mu[2],quad_form_diag(L*L', to_vector(sigma)))));
  y_new[n] = bernoulli_rng(p_new[n]);}
for (n in 1:N){
  p_pred[n]=exp(multi_normal_lpdf(x[n]|to_vector(mu[2]),quad_form_diag(L*L', to_vector(sigma))))/(exp(multi_normal_lpdf(x[n]|mu[1],quad_form_diag(L*L', to_vector(sigma))))+exp(multi_normal_lpdf(x[n]|mu[2],quad_form_diag(L*L', to_vector(sigma)))));
...
"""
  ...
  np.save("./npy_files/bayes_lda_zero.npy",{"fit":fit,"y_fitted_dist":y_fitted_dist,"y_predictive_dist":y_predictive_dist,"target_train":target_train,"target_test":target_test,"num_chains":4,"num_bayes_samples":1000,"num_warmup":1000,"divergences":divergences})
